...
def modify_redoc_related_files(result, domain_path, script_absolute_path, project_name):
    ...
    if not os.path.exists(os.path.join(result, 'src', 'a_Presentation', 'e_Redoc' 'RedocController.py')):
        shutil.copytree(os.path.join(script_absolute_path, 'apigenerator/resources/1 - Project/1 - BaseProject/Project/src/a_Presentation/e_Redoc'),
                        os.path.join(result, 'src', 'a_Presentation', 'e_Redoc'), dirs_exist_ok=True)
    ...
    for domain in domain_list:
        ...
        copy_file_and_replace_lines(domain_name,
                                    ...
                                    os.path.join(result, 'src', 'a_Presentation', 'e_Redoc', 'RedocController.py'))

    add_redoc_blueprint_register_function_to_redoc_controller(os.path.join(result, 'src', 'a_Presentation', 'e_Redoc', 'RedocController.py'))
